...
-- at.
data HoverIntent = HoverIntent { hoverPosition :: Position, hovering :: Bool }
...
instance Intent HoverIntent where
  enactIntent i flyer pos vel = do
    ...
    let dist = vectorSize $ desiredPos - (getVec pos)
    return i { hovering = (dist < 0.2) }
  success = hovering
